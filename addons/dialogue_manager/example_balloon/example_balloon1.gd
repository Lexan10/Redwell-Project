...
onready var response_template := $Responses/ResponseTemplate
...
func _ready() -> void:
	...
	balloon.anchor_left = 0
	balloon.anchor_top = 0
	balloon.anchor_right = 0
	balloon.anchor_bottom = 0
	balloon.rect_pivot_offset = Vector2.ZERO
	
	balloon.rect_min_size = Vector2(viewport_size.x * 0.96, 0)
	...
	var side = max(left_curr, right_curr)
	margin.set("custom_constants/margin_left", side)
	margin.set("custom_constants/margin_right", side)

	if top_curr == 0:  top_curr = 8
	if bot_curr == 0:  bot_curr = 8
	margin.set("custom_constants/margin_top", top_curr)
	margin.set("custom_constants/margin_bottom", bot_curr)

	var content_width = balloon.rect_min_size.x - float(side * 2)

	vbox.set("custom_constants/separation", 1)
	responses_menu.set("custom_constants/separation", 0)
	...
	dialogue_label.fit_content_height = true
	dialogue_label.scroll_active = false
	dialogue_label.rect_clip_content = false
	...
	if dialogue_line.responses.size() > 0:
		for i in range(dialogue_line.responses.size()):
			...
			item.set_meta("response_index", i)
			item.margin_left = 0
			item.margin_right = 0
			item.margin_top = 0
			item.margin_bottom = 0
			item.rect_min_size = Vector2(content_width, 14)
			item.scroll_active = false
			item.rect_clip_content = false
	...
	var max_h = float(viewport_size.y) * 0.36
	...
	if dialogue_line.responses.size() > 0:
		var responses_w = balloon.rect_min_size.x * 0.52
		responses_menu.rect_size.x = responses_w
		responses_menu.rect_global_position.x = balloon.rect_global_position.x + (balloon.rect_min_size.x - responses_w)
		var option_h = 14.0
		sep_val = responses_menu.get("custom_constants/separation")
		var option_sep = float(sep_val) if sep_val != null else 0.0
		var count = int(dialogue_line.responses.size())
		var options_h = float(count) * option_h + float(max(0, count - 1)) * option_sep + 6.0
		var gap = 4.0
		responses_menu.rect_global_position.y = balloon.rect_global_position.y - options_h - gap
	...
	if dialogue_line.responses.size() > 0:
		...
		for c in responses_menu.get_children():
			if c != response_template:
				c.visible = true
		configure_focus()
...
func _on_response_gui_input(event, item):
	if "disallowed" in item.name.to_lower():
		return

	var idx := int(item.get_meta("response_index"))
